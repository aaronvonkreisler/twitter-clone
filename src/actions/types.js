export const SET_ALERT = 'SET_ALERT';
export const REMOVE_ALERT = 'REMOVE_ALERT';
export const REGISTER_SUCCESS = 'REGISTER_SUCCESS';
export const REGISTER_FAIL = 'REGISTER_FAIL';
export const USER_LOADED = 'USER_LOADED';
export const AUTH_ERROR = 'AUTH_ERROR';
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS';
export const LOGIN_FAIL = 'LOGIN_FAIL';
export const LOG_OUT = 'LOG_OUT';
export const GET_CURRENT_USERS_PROFILE = 'GET_CURRENT_USERS_PROFILE';
export const GET_PROFILE = 'GET_PROFILE';
export const PROFILE_ERROR = 'PROFILE_ERROR';
export const ADD_TWEET = 'ADD_TWEET';

export const TWEETS_ERROR = 'TWEETS_ERROR';
export const DELETE_TWEET = 'DELETE_TWEET';
export const RETWEET_SUCCESS = 'RETWEET_SUCCESS';
export const UPDATE_FAVORITES = 'UPDATE_FAVORITES';
export const UPDATE_FAVORITES_FROM_STATUS = 'UPDATE_FAVORITES_FROM_STATUS';
export const GET_TWEET = 'GET_TWEET';
export const GET_TWEETS_REPLIES = 'GET_TWEETS_REPLIES';
export const GET_TWEETS_LIKED_USERS = 'GET_TWEETS_LIKED_USERS';
export const REPORT_TWEET = 'REPORT_TWEET';
export const REPLY_TO_TWEET_FROM_HOME = 'REPLY_TO_TWEET_FROM_HOME';
export const REPLY_TO_TWEET_FROM_STATUS = 'REPLY_TO_TWEET_FROM_STATUS';
export const PHOTO_UPLOAD_SUCCESS = 'PHOTO_UPLOAD_SUCCESS';
export const PHOTO_UPLOAD_ERROR = 'PHOTO_UPLOAD_ERROR';
export const GET_PROFILE_BY_USERNAME = 'GET_PROFILE_BY_USERNAME';
export const SELECTED_USER_LOADED = 'SELECTED_USER_LOADED';
export const GET_PROFILE_TWEETS = 'GET_PROFILE_TWEETS';
export const GET_PROFILE_REPLIES = 'GET_PROFILE_REPLIES';
export const GET_PROFILE_LIKES = 'GET_PROFILE_LIKES';
export const FOLLOW_USER = 'FOLLOW_USER';
export const UNFOLLOW_USER = 'UNFOLLOW_USER';
export const CLEAR_PROFILE = 'CLEAR_PROFILE';
export const EDIT_PROFILE = 'EDIT_PROFILE';
export const CLEAR_TWEET_STATE = 'CLEAR_TWEET_STATE';
export const SET_NAME_IN_PROFILE_DATA_STATE = 'SET_NAME_IN_PROFILE_DATA_STATE';
export const PROFILE_DATA_ERROR = 'PROFILE_DATA_ERROR';
export const FETCH_USERS_FOLLOWERS = 'FETCH_USERS_FOLLOWERS';
export const FETCH_USERS_FOLLOWING = 'FETCH_USERS_FOLLOWING';
export const CLEAR_PROFILE_DATA = 'CLEAR_PROFILE_DATA';
export const PREPARE_PROFILE_DATA = 'PREPARE_PROFILE_DATA';
export const UPLOAD_AVATAR_SUCCESS = 'UPLOAD_AVATAR_SUCCESS';
export const UPLOAD_COVER_PHOTO_SUCCESS = 'UPLOAD_COVER_PHOTO_SUCCESS';
export const UPLOAD_PHOTO_ERROR = 'UPLOAD_PHOTO_ERROR';
export const PIN_TWEET_TO_PROFILE = 'PIN_TWEET_TO_PROFILE';
export const REMOVE_PINNED_TWEET = 'REMOVE_PINNED_TWEET';
export const GET_PINNED_TWEET = 'GET_PINNED_TWEET';
export const ADD_TO_BOOKMARKS = 'ADD_TO_BOOKMARKS';
export const REMOVE_ALL_BOOKMARKS = 'REMOVE_ALL_BOOKMARKS';
export const GET_BOOKMARKS = 'GET_BOOKMARKS';
export const BOOKMARKS_ERROR = 'BOOKMARKS_ERROR';
export const CLEAR_BOOKMARKS = 'CLEAR_BOOKMARKS';
export const REMOVE_BOOKMARK_ITEM = 'REMOVE_BOOKMARK_ITEM';
export const OPEN_REPLY_MODAL = 'OPEN_REPLY_MODAL';
export const CLOSE_REPLY_MODAL = 'CLOSE_REPLY_MODAL';
export const SET_TWEET_IN_MODAL = 'SET_TWEET_IN_MODAL';
export const SIDEBAR_SUGGESTIONS_LOADED = 'SIDEBAR_SUGGESTIONS_LOADED';
export const SUGGESTIONS_ERROR = 'SUGGESTIONS_ERROR';
export const CLEAR_TIMELINE = 'CLEAR_TIMELINE';
export const FETCH_TWEETS_START = 'FETCH_TWEETS_START';
export const FETCH_TWEETS_SUCCESS = 'FETCH_TWEETS_SUCCESS';
export const FETCH_TWEETS_FAILURE = 'FETCH_TWEETS_FAILURE';
export const GET_SUGGESTED_USERS = 'GET_SUGGESTED_USERS';
export const CLEAR_CONNECT_USERS = 'CLEAR_CONNECT_USERS';
export const FETCH_INBOX_START = 'FETCH_INBOX_START';
export const FETCH_INBOX_SUCCESS = 'FETCH_INBOX_SUCCESS';
export const FETCH_INBOX_ERROR = 'FETCH_INBOX_ERROR';
export const CREATE_NEW_CHAT = 'CREATE_NEW_CHAT';
export const CHAT_ERROR = 'CHAT_ERROR';
export const SELECT_CHAT = 'SELECT_CHAT';
export const CLEAR_SELECTED_CHAT = 'CLEAR_SELECTED_CHAT';
export const FETCH_LIKES_START = 'FETCH_LIKES_START';
export const FETCH_LIKES_SUCCESS = 'FETCH_LIKES_SUCCESS';
export const CLEAR_LIKES = 'CLEAR_LIKES';
export const OPEN_GIF_MODAL = 'OPEN_GIF_MODAL';
export const CLOSE_GIF_MODAL = 'CLOSE_GIF_MODAL';
export const OPEN_MESSAGE_MODAL = 'OPEN_MESSAGE_MODAL';
export const CLOSE_MESSAGE_MODAL = 'CLOSE_MESSAGE_MODAL';
export const GET_PROFILE_FOLLOW_SUGGESTIONS = 'GET_PROFILE_FOLLOW_SUGGESTIONS';
export const GET_CHAT_WITH_SPECIFIC_USER = 'GET_CHAT_WITH_SPECIFIC_USER';
export const SEND_DM_START = 'SEND_DM_START';
export const SEND_DM_SUCCESS = 'SEND_DM_SUCCESS';
export const DM_ERROR = 'DM_ERROR';
export const FETCH_MESSAGES_SUCCESS = 'FETCH_MESSAGES_SUCCESS';
export const FETCH_MESSAGES_START = 'FETCH_MESSAGES_START';
export const CLEAR_MESSAGES = 'CLEAR_MESSAGES';
export const UPDATE_MESSAGES = 'UPDATE_MESSAGES';
export const CONNECT_SOCKET = 'CONNECT_SOCKET';
export const DISCONNECT_SOCKET = 'DISCONNECT_SOCKET';
export const FETCH_NOTIFICATIONS_START = 'FETCH_NOTIFICATIONS_START';
export const FETCH_NOTIFICATIONS_SUCCESS = 'FETCH_NOTIFICATIONS_SUCCESS';
export const NOTIFICATIONS_ERROR = 'NOTIFICATIONS_ERROR';
